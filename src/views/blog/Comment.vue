<template>
    <div>
        <div class='comment-item' v-for="(item,index) in props.data" :key="index">
            <span class='comment-item-img'
                :style="{'backgroundImage':` url('${item.parent_avatar}')`}" />
            <div class='comment-item-box'>
                <div class='comment-item-context'>
                    <h3>{{ item.parent_name }}<span>{{ moment(item.create_time).format('YYYY-MM-DD') }}</span></h3>
